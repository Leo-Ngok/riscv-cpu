...
module instr_cache(
   ...
   typedef struct packed{
    ...
    cache_state_t state;
    reg [$clog2(N_WAYS) - 1: 0] fetch_way_idx_comb;
            ...
            FETCHING_BLOCKS: begin
                if(dau_iack) begin
                    if(fetch_block_idx == BLOCK_COUNT - 1) begin
                        state <= WAIT;
                        fetch_block_idx <= 0;
                        cache_regs[set_idx].way_arr[fetch_way_idx_comb].valid <= 1'b1;
                        cache_regs[set_idx].way_arr[fetch_way_idx_comb].tag <= tag;
                    end else begin
                        fetch_block_idx <= fetch_block_idx + 1;
                    end
                    cache_regs[set_idx].way_arr[fetch_way_idx_comb].blocks[fetch_block_idx] <= dau_idata;
                end
            end
